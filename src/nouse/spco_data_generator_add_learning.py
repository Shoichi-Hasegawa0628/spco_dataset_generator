...
class SpCoDataGenerator():
    ...
    def set_param(self):
        get_key = input("Please input the number of add data\n")
        print("The number of add data is {}\n".format(get_key))
    ...
    def place_word_generator(self, num):
        ...
        for i in range(num):
            path = PLACE_WORD_DATA + str(i + 1 + 60)
    ...
    def object_frequency_generator(self, num):
        ...
        for i in range(num):
            ...
            ## 物体の単語辞書
            with open(OBJECT_FREQUENCY_DATA + '{}_Object_W_list.csv'.format(i + 1 + 60), 'w') as f:
                writer = csv.writer(f, lineterminator='\n')
                writer.writerow(object_dictionary)

            ## 観測した物体
            with open(OBJECT_FREQUENCY_DATA + '{}_Object.csv'.format(i + 1 + 60), 'a') as f:
                for j in range(i + 1):
                    writer = csv.writer(f)
            ...
            # 全物体の観測画像化
            ## BOO の書き出し
            with open(OBJECT_FREQUENCY_DATA + '{}_Object_BOO.csv'.format(i + 1 + 60), 'a') as f:
                for j in range(i+1):
                    writer = csv.writer(f)
