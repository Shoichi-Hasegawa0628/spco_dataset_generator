...
class ObjectFeatureServer():
    ...
    def extracting_label(self):
        ...
        for i in range(len(self.detect_object_info)):
            if self.detect_object_info[i].Class == "frog_shaped_sponge" or self.detect_object_info[i].Class == "chips_bag" or self.detect_object_info[i].Class == "muscat":
                pass
            else:
                object_list.append(self.detect_object_info[i].Class)
            # print(object_list)
        self.object_list.append(object_list)
        self.object_list = [a for a in self.object_list if a != []]
